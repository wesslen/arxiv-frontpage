...
def fetch_and_filter_articles(nlp, query="cs", max_results=500, days_limit=2.5):
    ...
    logger.info(
        f"Found {len(results)} new results within {days_limit} days in the {query} category."
    )
    return results
...
def save_new_articles(new_articles):
    ...
    if new_articles:
        filename = (
            f"{dt.datetime.now().replace(microsecond=0).isoformat()}-articles.jsonl"
        )
...
def main():
    ...
    parsed_articles = [
        parse_article_result_to_dict(article, nlp=nlp)
        for article in tqdm.tqdm(fetched_articles)
    ]

    # load previously saved articles to check for duplicates
    most_recent_file = sorted(Path("data/downloads").glob("*.jsonl"))[-1]
    old_articles_dict = {
        art["title"]: art for art in srsly.read_jsonl(most_recent_file)
    }

    new_articles = [
        art for art in parsed_articles if art["title"] not in old_articles_dict
    ]
