...
from .constants import DATA_LEVELS, INDICES_FOLDER, LABELS, CONFIG, THRESHOLDS, CLEAN_DOWNLOADS_FOLDER, DOWNLOADS_FOLDER, ANNOT_PATH, ACTIVE_LEARN_PATH, SECOND_OPINION_PATH, ANNOT_FOLDER, EVAL_FOLDER, DATA_TYPES, DATA_PARTITION
from .modelling import SentenceModel
from .utils import console, dedup_stream, add_rownum, attach_docs, attach_spans, add_predictions, abstract_annot_to_sent, str_to_probability
import warnings
...
class DataStream:
    ...
    def save_clean_download_stream(self):
        ...
        nested = (LazyLines(stream)
                    .mutate(data_type=lambda d: "evaluation" if str_to_probability(d['title']) < DATA_PARTITION else "training")
                    .mutate(created=lambda d: d['created'][:10])
                    .nest_by("created"))
        for group in nested:
    ...
    def get_download_stream(self, level:str="sentence"):
        ...
        abstract_stream = ({"text": ex["abstract"], "sentences": ex["sentences"], "created": ex["created"][:10], "meta": {"created": ex["created"][:10], "url": ex["url"], "title": ex["title"], "data_type": ex["data_type"]}} 
                           for ex in stream)
        sentences_stream = ({"text": sent, "meta": {"url": ex["url"], "data_type": ex["data_type"]}} 
    ...
    def save_train_stream(self):
        ...
        for dataset in self.retreive_dataset_names():
            ...
            if "evaluation" not in dataset:
                datapoints = self._filter_datatype(datapoints, "training")
    ...
    def save_eval_stream(self):
        ...
        for dataset in self.retreive_dataset_names():
            if "evaluation" in dataset:
                datapoints = self.db.get_dataset_examples(dataset)
                datapoints = self._filter_datatype(datapoints, "evaluation")
                if "sentence" in dataset:
                    new = list(self._sentence_data_to_train_format(datapoints))
                    console.log(f"Adding {len(new)} examples from {dataset}")
                    stream.extend(new)
    ...
    def get_lunr_stream(self, query: str, level: str, data_type="training"):
        idx_path = self._index_path(kind="lunr", level=level, data_type=data_type)
        ...
        documents = (LazyLines(self.get_download_stream(level=level))
                     .keep(lambda d: d["meta"]["data_type"] == data_type)
                     .pipe(add_rownum)
                     .collect())
        return [documents[int(i['ref'])] for i in idx.search(query)]

    def get_ann_stream(self, query: str, level: str, data_type="training"):
        from simsity import load_index
        model = SentenceModel()
        idx = load_index(self._index_path(kind="simsity", level=level, data_type=data_type), encoder=model.encoder)
    ...
    def build_active_learn_stream(self, n=5000, data_type="training"):
        ...
        out = LazyLines(stream).keep(lambda d: d["meta"]["data_type"] == data_type).head(n).progress().pipe(add_preds).collect()
    ...
    def build_second_opinion_stream(self, n=2000, data_type="training"):
        ...
        out = LazyLines(stream).keep(lambda d: d["meta"]["data_type"] == data_type).head(n).progress().pipe(add_predictions, model=model).collect()
        
        srsly.write_jsonl(SECOND_OPINION_PATH, out)
        console.log(f"Predictions saved in {SECOND_OPINION_PATH}.")

    def get_second_opinion_stream(self, label, min_sents=1, max_sents=5, data_type="training"):
    ...
    def get_random_stream(self, level:str, data_type:str):
        return (ex for ex in self.get_download_stream(level=level) if (random.random() < 0.05) & (ex['meta']['data_type'] == data_type))

    def _index_path(self, kind:str, level:str, data_type:str) -> Path:
        """kind is lunr vs. simsity, level is sentence vs. abstract"""
        path = INDICES_FOLDER / kind / level / data_type
        if kind == "simsity":
            return path
    ...
    def create_lunr_index(self, level:str):
        for data_type in DATA_TYPES:
            console.log(f"Preparing lunr index for {level}-{data_type}")
            stream = LazyLines(self.get_download_stream(level=level)).keep(lambda d: d["meta"]["data_type"] == data_type).pipe(add_rownum).collect()
            index = lunr(ref='idx', fields=('text',), documents=stream)
            serialized = index.serialize()
            with open(self._index_path(kind="lunr", level=level, data_type=data_type), 'w') as fd:
                json.dump(serialized, fd)
            console.log(f"Lunr index for  {level}-{data_type} created")
    
    def create_simsity_index(self, level:str):
        from simsity import create_index
        for data_type in DATA_TYPES:
            model = SentenceModel()
            stream = LazyLines(self.get_download_stream(level=level)).keep(lambda d: d["meta"]["data_type"] == data_type).map(lambda d: d['text']).collect()
            console.log(f"Preparing simsity index for {level}-{data_type} with {len(stream)} examples.")
            path = self._index_path(kind="simsity", level=level, data_type=data_type)
    ...
    def create_indices(self):
        ...
        for level in DATA_LEVELS:
            for data_type in DATA_TYPES:
                self.create_simsity_index(level=level,data_type=data_type)
                self.create_lunr_index(level=level,data_type=data_type)
            ...
            for label in LABELS:
                for data_type in DATA_TYPES:
                    ...
                    if data_type == "evaluation":
                        dataset_name = f"{label}-{level}-{data_type}"
