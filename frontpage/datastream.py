...
class DataStream:
    ...
    def save_clean_download_stream(self):
        ...
        nested = (
            LazyLines(stream)
            .mutate(
                data_type=lambda d: "evaluation"
                if str_to_probability(d["title"]) < DATA_PARTITION
                else "training"
            )
        ...
        console.log(
            f"Cleaned files written in [bold]{CLEAN_DOWNLOADS_FOLDER}[/bold] folder."
        )
    ...
    def get_download_stream(self, level: str = "sentence"):
        ...
        sentences_stream = (
            {"text": sent, "meta": {"url": ex["url"], "data_type": ex["data_type"]}}
            for ex in stream
    ...
    def _sentence_data_to_train_format(self, stream):
        ...
        for ex in stream:
            ...
            if outcome is not None:
                yield {"text": ex["text"], ex["label"]: outcome}
    ...
    def get_lunr_stream(self, query: str, level: str, data_type="training"):
        ...
        documents = (
            LazyLines(self.get_download_stream(level=level))
    ...
    def get_ann_stream(self, query: str, level: str, data_type="training"):
        ...
        model = SentenceModel()
        idx = load_index(
            self._index_path(kind="simsity", level=level, data_type=data_type),
    ...
    def get_second_opinion_stream(
        self, label, min_sents=1, max_sents=5, data_type="training"
    ):
    ...
    def get_random_stream(self, level: str, data_type: str):
        return (
            ex
            for ex in self.get_download_stream(level=level)
            if (random.random() < 0.05) & (ex["meta"]["data_type"] == data_type)
        )
    ...
    def create_lunr_index(self, level: str):
        for data_type in DATA_TYPES:
            ...
            with open(
                self._index_path(kind="lunr", level=level, data_type=data_type), "w"
            ) as fd:
    ...
    def create_simsity_index(self, level: str):
        ...
        for data_type in DATA_TYPES:
            ...
            console.log(
                f"Preparing simsity index for {level}-{data_type} with {len(stream)} examples."
            )
            ...
            for ex in stream:
                for preds in ex["preds"]:
                    for name, proba in preds.items():
                        if (
                            name in tracker
                            and proba > THRESHOLDS[name]
                            and tracker[name] < limit
                        ):
                            tracker[name] += 1
    ...
    def get_site_content(self):
        ...
        sections = {
            dict(section)["label"]: {**dict(section), "content": []}
        ...
        for section in sections.keys():
            ...
            sections[section]["content"] = reversed(
                sorted(uniq_content, key=lambda d: d["created"])
            )
