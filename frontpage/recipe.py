...
def arxiv_sentence(dataset, label, data_type, tactic, setting):
    ...
    if tactic == "simsity":
        ...
        stream = datastream.get_ann_stream(
            query=setting, level="sentence", data_type=data_type
        )
    elif tactic == "random":
        ...
        stream = datastream.get_active_learn_stream(
            label=label, preference=setting, data_type=data_type
        )
    elif tactic == "search-engine":
        console.log("Setting up lunr query")
        stream = datastream.get_lunr_stream(
            query=setting, level="sentence", data_type=data_type
        )
    else:
    ...
    return {
        ...
        "config": {"exclude_by": "input"},
    }


@prodigy.recipe(
    "textcat.arxiv.abstract",
    dataset=("The dataset to save", "positional", None, str),
    label=("The label to annotate", "positional", None, str),
    data_type=("The data partition: training or evaluation", "positional", None, str),
    tactic=("The tactic to retreive relevant examples", "positional", None, str),
...
def arxiv_abstract(dataset, label, data_type, tactic, setting):
    ...
    if tactic == "simsity":
        console.log("Setting up simsity stream")
        stream = datastream.get_ann_stream(
            query=setting, level="abstract", data_type=data_type
        )
    elif tactic == "random":
        console.log("Setting up randomized stream")
        ...
        stream = datastream.get_second_opinion_stream(
            label=label, min_sents=1, max_sents=2, data_type=data_type
        )
    else:
        raise ValueError("This should never happen.")
...
def annotate_prodigy(results):
    ...
    dataset_name = datastream.get_dataset_name(
        results["label"], results["level"], results["data_type"]
    )
    name = (
        "textcat.arxiv.sentence"
        if results["level"] == "sentence"
        else "textcat.arxiv.abstract"
    )
    if results["level"] == "sentence":
        ctrl_data = arxiv_sentence(
            dataset_name,
            results["label"],
            results["data_type"],
            results["tactic"],
